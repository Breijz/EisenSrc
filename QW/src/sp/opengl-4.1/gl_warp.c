...
void EmitWaterPolys (msurface_t *fa)
{
	...
	for (p=fa->polys ; p ; p=p->next)
	{
		...
		{
			initialbuff = malloc(sizeof(float) + 5);
			if(!initialbuff) {
				fprintf(stderr, "Failed to malloc intermediate buffer\n");
			}
			size += 5;
			...
			initialbuff[size-5] = s;
			initialbuff[size-4] = t;
			initialbuff[size-3] = *v;
		}
	}
	glBufferData(GL_ARRAY_BUFFER, sizeof(initialbuff), initialbuff, GL_STATIC_DRAW);
